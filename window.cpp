...
Window::Window() : gain(5), count(0)
{
   ...
   connect(pausebutton, SIGNAL(clicked()), this, SLOT(pauseslot()));
...
{  
	 ...
	 vLayout->addWidget(resumebutton);
	 vLayout->addWidget(stopbutton); 
	 vLayout->addWidget(quitbutton); 
	 
	 player = new QMediaPlayer;
	 player->setMedia(QUrl::fromLocalFile("/home/jiangjiajun/Testing0/C-scale.mp3"));
	 player->setVolume(50);           // set the volume 
     player->play();                 //play the song
	 pushbutton1->hide();     //hide every button in the app while listening to avoid conflicts
	 pushbutton2->hide();
	 pushbutton3->hide();
	pausebutton->show();     
	resumebutton->show();
...
//this function is used to stop the learning video then back to the first window to enable the user to press any button
void Window::stopslot()  
{ 
    ...
    pausebutton->hide();     //hide every button in the app while listening to avoid conflicts
	resumebutton->hide();
	stopbutton->hide();
